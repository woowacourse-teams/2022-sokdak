...
class HotBoardNotificationEventHandlerTest {

    @DisplayName("Hot Board 알림에 대한 NotificationEvent를 Notification으로 파싱한다.")
    @Test
    void parse() {
        NotificationEvent notificationEvent = NotificationEvent.hotBoard(CHRIS_POST, CHRIS);
        NotificationHandler notificationHandler = new HotBoardNotificationEventHandler();
        ...
        assertAll(
                ...
                () -> assertThat(notification.getNotificationType()).isEqualTo(NotificationType.HOT_BOARD)
        );
    }
}
